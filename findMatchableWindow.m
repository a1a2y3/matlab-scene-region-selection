function findMatchableWindowMultiSensor(im,RefW,RefH,winW,winH,step)
%% 参数准备
if nargin<6
    step=100;
end
...
for i=1:N3
    ...
    ratio = 100/sqrt(ImgRefSize_h*ImgRefSize_w);
    win3= imresize(win3,ratio);
    win3_s= imfilter(win3,h);
    win3_a= imnoise(win3,'speckle',0.4);    
    [mRate,~]= CrossMatchRate_BSC0(win3_s,win3_a,winW,winH);
...
saveas(gcf,'..\saveresult\region_find_multisensor.emf');
fig1= figure;
for i=1:N4
    k= idx3(i);
